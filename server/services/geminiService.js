...
const CONTRACT_PROMPT = (contractType, formData) => `You are a senior legal drafting AI. Generate a complete, formal ${contractType} contract using the following party and agreement details:
...
const RISK_PROMPT = (contractText) => `You are a legal risk analyst. Analyze the following contract for legal risks, imbalances, and problematic clauses.
...
const SUMMARY_PROMPT = (contractText) => `Explain the following contract in plain English for someone with no legal background.
...
export const geminiService = {
  async generateContract(contractType, formData) {
    return runPrompt(CONTRACT_PROMPT(contractType, formData));
  },

  async analyzeRisk(contractText) {
    const output = await runPrompt(RISK_PROMPT(contractText));
  ...
  async summarize(contractText) {
    return runPrompt(SUMMARY_PROMPT(contractText));
  },
};
